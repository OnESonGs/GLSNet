...
class DNCNN(nn.Module):
    def __init__(self):
        ...
        ch = 96
        # CONVOLUTION LAYERS
        self.c1 = nn.Conv1d(in_channels=1, out_channels=ch, kernel_size=3, padding=1)
        self.a1 = nn.ReLU(inplace=True)
        self.c2 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b2 = nn.BatchNorm1d(num_features=ch)
        self.a2 = nn.ReLU(inplace=True)
        self.c3 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b3 = nn.BatchNorm1d(num_features=ch)
        self.a3 = nn.ReLU(inplace=True)
        self.c4 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b4 = nn.BatchNorm1d(num_features=ch)
        self.a4 = nn.ReLU(inplace=True)
        self.c5 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b5 = nn.BatchNorm1d(num_features=ch)
        self.a5 = nn.ReLU(inplace=True)
        self.c6 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b6 = nn.BatchNorm1d(num_features=ch)
        self.a6 = nn.ReLU(inplace=True)
        self.c7 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b7 = nn.BatchNorm1d(num_features=ch)
        self.a7 = nn.ReLU(inplace=True)
        self.c8 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b8 = nn.BatchNorm1d(num_features=ch)
        self.a8 = nn.ReLU(inplace=True)
        self.c9 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b9 = nn.BatchNorm1d(num_features=ch)
        self.a9 = nn.ReLU(inplace=True)
        self.c10 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b10 = nn.BatchNorm1d(num_features=ch)
        self.a10 = nn.ReLU(inplace=True)
        self.c11 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b11 = nn.BatchNorm1d(num_features=ch)
        self.a11 = nn.ReLU(inplace=True)
        self.c12 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b12 = nn.BatchNorm1d(num_features=ch)
        self.a12 = nn.ReLU(inplace=True)
        self.c13 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b13 = nn.BatchNorm1d(num_features=ch)
        self.a13 = nn.ReLU(inplace=True)
        self.c14 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b14 = nn.BatchNorm1d(num_features=ch)
        self.a14 = nn.ReLU(inplace=True)
        self.c15 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b15 = nn.BatchNorm1d(num_features=ch)
        self.a15 = nn.ReLU(inplace=True)
        self.c16 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b16 = nn.BatchNorm1d(num_features=ch)
        self.a16 = nn.ReLU(inplace=True)
        self.c17 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b17 = nn.BatchNorm1d(num_features=ch)
        self.a17 = nn.ReLU(inplace=True)
        self.c18 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b18 = nn.BatchNorm1d(num_features=ch)
        self.a18 = nn.ReLU(inplace=True)
        self.c19 = nn.Conv1d(in_channels=ch, out_channels=ch, kernel_size=3, padding=1)
        self.b19 = nn.BatchNorm1d(num_features=ch)
        self.a19 = nn.ReLU(inplace=True)
        self.c20 = nn.Conv1d(in_channels=ch, out_channels=1, kernel_size=3, padding=1)
...
# LOSS FUNCTION
'''
INPUT:  y_p - PREDICTION, y_true - LABELS 
'''
...
def get_evm_func_for_mid(indices, prediction, x, y):
    ...
    print('(Linear) Original EVM: %.10f' % eevm0)
    print('(Linear) Post-EVM:     %.10f' % eevm)

    eevm0 = np.mean(10 * np.log10(evm0))
    eevm = np.mean(10 * np.log10(evm))
    print('(Non-linear) Original EVM: %.10f' % eevm0)
    print('(Non-linear) Post-EVM:     %.10f' % eevm)
...
def train_dncnn(m, margin, db_value, channel_name):
    ...
    mode = mode_list[m]  # RUNNING MODE

    seed = 777777  # RANDOM SEED
    np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    train_dncnn(0, 0, 40, 'ChB')
    train_dncnn(0, 0, 30, 'ChB')
