...
from plot_subcarriers_SNR import mean_confidence_interval
...
def plotfunc_mse(arr_list, model_list, channel_name):
    ...
    fill_colors = [(0.39, 0.39, 0.39) for r, g, b in main_colors]
    ...
    for idx, (mse_data, model_name) in enumerate(zip(arr_list, model_list)):
        ...
        ax.plot(x_axis, mse_mean, linestyle='--', marker=marker_list[idx], label=model_name, color=main_color, linewidth=2.0)

    plt.xlabel(f'SNR of Original Signal in {channel_name}', fontsize=14)
    plt.ylabel(f'MSE of Filtered Signal in {channel_name}', fontsize=14)
    plt.yscale('log')
    ...
    # plt.legend()
    plt.legend(loc='upper right', fontsize=14, framealpha=1, edgecolor='black')
