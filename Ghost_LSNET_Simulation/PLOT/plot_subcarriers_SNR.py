...
def plot_func(ans_list, model_list, channel_name, db_value):
    ...
    for idx, (snr_data, model_name) in enumerate(zip(ans_list, model_list)):
        ...
        ax.plot(
            ...
            color=main_color, label=model_name,
            linewidth=2.0 # , marker='.', markersize=2
        )

    ax.set_xlabel('Sub-carrier Index', fontsize=14, fontweight='bold')
    ax.set_ylabel(f'The SNR of Sub-carriers (dB) in {channel_name}', fontsize=14, fontweight='bold')
    ...
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, 0.3), fontsize=14, framealpha=1, edgecolor='black')
    plt.savefig(
