...
class LKP1d(nn.Module):
    ...
    def forward(self, x):
        residual = self.cv1(x) # zone debug
...
class LSGhostNet_1D(nn.Module):
    def __init__(self, input_length=1001, ch=64):
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(1, ch, kernel_size=9, padding=(9-1)//2),  # 保持L不变
            nn.BatchNorm1d(ch),
        ...
        # self.cv1 = nn.Conv1d(2*ch, 2*ch, kernel_size=5, padding=(5-1)//2),  # 保持L不变
        # self.feaupsample2 = LSConv(2*ch, 3)  # ch -> 2*ch
    ...
    def forward(self, x):
        ...
        # x3 = self.feaupsample2(x2)  # [B,128,L]
...
(线性)后EVM: -17.0368011396
(非线性)原EVM: 0.2230050456
(非线性)后EVM: -17.5598665467

(线性)原EVM: -9.3352297515
(线性)后EVM: -25.8296755664
(非线性)原EVM: -9.7725637779
(非线性)后EVM: -26.3499632244

(线性)原EVM: -19.3666204420
(线性)后EVM: -34.5029279370
(非线性)原EVM: -19.7927158973
(非线性)后EVM: -34.9835136989

(线性)原EVM: -29.3708968574
(线性)后EVM: -42.9813948538
(非线性)原EVM: -29.7951247538
(非线性)后EVM: -43.4466067801

(线性)原EVM: -39.3999340603
(线性)后EVM: -51.2301322844
(非线性)原EVM: -39.8083702858
(非线性)后EVM: -51.7387431215
'''